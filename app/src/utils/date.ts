...
const getPart = (parts: Intl.DateTimeFormatPart[], type: Intl.DateTimeFormatPart['type']) =>
...
/** YYYY-MM-DD (KST) */
export const formatKstYYYYMMDD = (input?: string | number | Date, sep: '-' | '.' | '/' = '-'): string => {
  const d = input ? new Date(input) : new Date();
...
export const kstDateKey = (input?: string | number | Date): string => formatKstYYYYMMDD(input, '-');
...
export const isSameKstDay = (a?: string | number | Date, b?: string | number | Date): boolean =>
  kstDateKey(a) === kstDateKey(b);
