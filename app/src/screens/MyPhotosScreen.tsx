...
const normalizeUri = (u?: string) =>
  !u
    ...
    : u.startsWith("http")
      ? u
      : `${BASE_URL.replace(/\/$/, "")}/${String(u).replace(/^\//, "")}`;
          ...
          <View style={styles.missionBox}>
            ...
            <View>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <MissionInfo
                  mission={
                    typeof item.missionId === "object"
                      ? (item.missionId as Mission)
                      : undefined
                  }
                  compact
                />
...
const MyPhotosScreen = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const normalized = arr.map((it: any) => ({
        ...it,
        missionId: normalizeMission(it.missionId) ?? it.missionId,
      }));
  ...
  const ListFooter = useMemo(
    () => <View style={{ height: contentPadBottom }} />,
    [contentPadBottom],
  );
    ...
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? (
            <View
              style={[styles.emptyWrap, { paddingBottom: contentPadBottom }]}
            >
              <EmptyState
                title="아직 업로드한 사진이 없어요"
                subtitle={"오늘의 미션을 완료하고\n첫 사진을 올려보세요!"}
              />
            </View>
          ) : null
        }
