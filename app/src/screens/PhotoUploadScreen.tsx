...
const PhotoUploadScreen = () => {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('업로드 완료', result?.replaced ? '오늘 올린 사진이 있어 새 사진으로 교체됐어요.' : '오늘의 사진이 등록됐어요.');
...
const styles = StyleSheet.create({
  ...
  missionTextLoading: { marginLeft: spacing.md, color: colors.text, fontSize: 13, marginBottom: spacing.sm },
  missionTitle: {
    color: colors.text,
