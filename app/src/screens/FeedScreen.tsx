...
import { BASE_URL, missionAPI, photoAPI } from "../services/api";
...
const FeedScreen = memo(() => {
  ...
  const mapPhotoToItem = (p: any): FeedItem => {
    ...
    const missionObj: Mission | undefined = normalizeMission(p?.missionId) as
      | Mission
      | undefined;
  ...
  const loadInitial = async () => {
    try {
      ...
      if (active === "오늘의 미션") {
        ...
        const photos = mission?._id
          ? await photoAPI.getPhotosByMission?.(mission._id)
          : [];
      ...
      return (
        <Card style={styles.postCard}>
          ...
          <View style={styles.postHeader}>
            ...
            <Pressable style={styles.moreBtn} accessibilityLabel="더보기">
                ...
                {tabs.map((t) => (
                  ...
                  >
                    <Text
                      style={[
                        styles.tabLabel,
                        active === t && styles.tabLabelActive,
                      ]}
                    >
                      {t}
                    </Text>
