...
async function main() {
  console.warn(
    '⚠️ Running mission backfill. Make sure you run this against a development DB only.',
  );
  ...
  try {
    ...
    const missing = await (svc as any).missionModel
      .find({
        $or: [
    ...
    for (const m of missing) {
      ...
      const description =
        m.description ||
        `${title.replace(/^\s*/, '')}를 주제로 사진을 찍어보세요!`;
      await (svc as any).missionModel
        .updateOne({ _id: m._id }, { $set: { subtitle, description } })
        .exec();
